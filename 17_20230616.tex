...
\begin{proof}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \begin{enumerate}[label=(\roman*)]
            ...
            \begin{align}
                Q_{t}f(x)
                = \int_{C(\mathbb{R_+}, \mathbb{R}^{m})}f(F_{x}(w)_{t})W(dw)
            \end{align}
            と優収束定理より $Q_{t}f$: 連続\nazo．
...
確率微分方程式の解として得られる連続な sample paths を持つ Markov 過程を diffusion process（拡散過程）と呼ぶことがある（ある著者は $\mathbb{R}^d$ または多様体上の連続な sample paths を持つ強 Markov 過程を拡散過程と呼ぶ）．
なお，ここで考える Lipschitz の設定でも，Theorem 8.7 は生成作用素 $L$ を完全に特定するものではなく，領域 $D(L)$ の部分集合に対する作用に過ぎない：Chapter 6 で既に述べたように，領域を完全に記述することはしばしば非常に困難である．
